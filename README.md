...
-DCMAKE_INSTALL_PREFIX=$PWD/build/sim_ios \
-Ssrc -Bbuild -G Xcode && \
...
-DCMAKE_OSX_DEPLOYMENT_TARGET="13.0" \
-DCMAKE_OSX_ARCHITECTURES=arm64 \
-DCMAKE_INSTALL_PREFIX=$PWD/build/ios \
-Ssrc -Bbuild -G Xcode && \
cmake --build build --config Release -t install -j$(nproc)
# xcframework
xcodebuild -create-xcframework \
-library "build/ios/lib/libyolo_ffi.a" \
-headers build/ios/include \
-library "build/sim_ios/lib/libyolo_ffi.a" \
-output "ios/Frameworks/yolo_ffi.xcframework"
```
